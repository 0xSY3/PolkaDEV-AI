...
mod erc20 {
    ...
    impl Erc20 {
        ...
        pub fn transfer_from(
            ...
            if allowance < value || self.balance_of(from) < value {
                return false;
            }
            self.transfer_from_to(&from, &to, value);
...
export const createConfigFile = async (contract: string) => {
  const chat_completion = await openai.chat.completions.create({
    ...
    messages: [{
      role: "system",
      content: `
      For the go-lang smart contract written using neo-go framework for Neo Blockchain, write the correct .yml file, here's some examples for syntax (don't create random types by yourself)
      ...
      + Return only the configuration text, don't write any comments, suggestions, or knowledge about it.
      + The configuration yml should be designed correctly as per syntax and the contract
    `}, {
      role: "user", content: `
